...
class select_action(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name, robot_index):
        ...
        self.blackboard.register_key(key="hm_carry_to_zero", access=py_trees.common.Access.WRITE)
    ...
    def setup(self): 
        ...
        self.blackboard.hm_carry_to_zero = []
    ...
    def update(self):
        ...
        # Pick block
            ...
            if all(item['status'] == 1 for item in self.blackboard.local_task_log.values() if item['colour'] == box.colour) or all(item['carry_status'] != [0, 0] for item in self.blackboard.local_task_log.values() if item['colour'] == box.colour):            
                self.blackboard.action = 'random_walk'
                self.blackboard.target_box = None
                self.blackboard.carrying_box = False
            else:
        ...
        # Pre-place block
            ...
            if (self.blackboard.local_task_log[id]['status'] == 1 or self.blackboard.local_task_log[id]['carry_status'][0] not in {self.str_index}): 

                # If box hasbeen placed, update carry status 
                if self.blackboard.local_task_log[id]['status'] == 1:
                    self.blackboard.local_task_log[id]['carry_status'] = [0, 0]
                    self.blackboard.hm_carry_to_zero.append(self.blackboard.target_place_id)
                ...
                for id, value in self.blackboard.local_task_log.items():
                    if value['colour'] == box.colour and value['status'] == 0 and value['carry_status'] == [0, 0]:
                        # Box need placing elsewhere - continue to carry
                        self.blackboard.target_place_id = id
                        self.blackboard.local_task_log[id]['carry_status'][0] = self.str_index
                        self.blackboard.local_task_log[id]['carry_status'][1] = time.time()
                        break
                    else:
                        self.blackboard.target_place_id = None
        ...
        # Place block
            ...
            # Check if box has been placed or is being carried by another agent
            if (self.blackboard.local_task_log[id]['status'] == 1 or self.blackboard.local_task_log[id]['carry_status'][0] not in {self.str_index, 0}):

                # If box has been placed, update carry status
                if self.blackboard.local_task_log[id]['status'] == 1:
                    self.blackboard.local_task_log[id]['carry_status'] = [0,0]
                    self.blackboard.hm_carry_to_zero.append(self.blackboard.target_place_id)
                ...
                for id, value in self.blackboard.local_task_log.items():
                    if value['colour'] == box.colour and value['status'] == 0 and value['carry_status'] == [0, 0]:
                        ...
                        self.blackboard.target_place_id = id
                        self.blackboard.local_task_log[id]['carry_status'] = []
                        self.blackboard.local_task_log[id]['carry_status'][0] = self.str_index
                        self.blackboard.local_task_log[id]['carry_status'][1] = time.time()
                        break
                    else:
            ...
            # Otherwise continue checking place position
                ...
                # Block placed
                    ...
                    self.blackboard.local_task_log[self.blackboard.target_place_id]['carry_status'] = [0,0]
                    self.blackboard.hm_carry_to_zero.append(self.blackboard.target_place_id)
                    self.blackboard.target_place_id = None
                    self.blackboard.carrying_box = False
...
class look_for_blocks(py_trees.behaviour.Behaviour):
    ...
    def __init__(self,name, robot_index):
        ...
        self.blackboard.register_key(key="use_hm", access=py_trees.common.Access.READ)
        self.setup()
                    ...
                    for id, value in self.blackboard.local_task_log.items():
                        ...
                        if value['colour'] == box.colour and value['status'] != 1:
                            ...
                            if distance < self.blackboard.place_tol:
                                ...
                                if self.blackboard.use_hm == 'None':
...
class local_communication(py_trees.behaviour.Behaviour): 
    '''
    '''
    def __init__(self, name, robot_index):
        super().__init__(name)
        ...
        self.blackboard.register_key(key="boxes", access=py_trees.common.Access.WRITE)
        self.blackboard.register_key(key="local_task_log", access=py_trees.common.Access.WRITE)
        self.blackboard.register_key(key="local_comms", access=py_trees.common.Access.READ)
        self.blackboard.register_key(key="local_comms_range", access=py_trees.common.Access.READ)
        self.setup()

    def setup(self):
        self.logger.debug(f"Select local communication::setup {self.name}")
        
    def initialise(self):
        self.logger.debug(f"select local communication::initialise {self.name}")

    def get_neighbour_task_data(self, neighbour_index):
        # To simulate communication here we simply access the neighbours task log directly, as if it had been sent over
        neighbour_str = 'robot_' + str(neighbour_index)
        self.neighbour_blackboard = self.attach_blackboard_client(name='neighbour_bb', namespace=str(neighbour_str))
        self.neighbour_blackboard.register_key(key="local_task_log", access=py_trees.common.Access.READ)

        return self.neighbour_blackboard.local_task_log
    
    def update_task_status(self, id, new_task_status=None, new_carry_status=None):
        if new_task_status is not None:
            self.blackboard.local_task_log[id]['status'] = new_task_status
        if new_carry_status is not None:
            self.blackboard.local_task_log[id]['carry_status'] = new_carry_status

        
    def update(self):
        # Using local comms
        if self.blackboard.local_comms:
            # Calculate distance to all other agents
            agent_dist = [euclidean_agents(self.blackboard.rob_c[self.robot_index], coord) for coord in self.blackboard.rob_c]

            # Check if distance of other agents are within comms range
            for index, dist in enumerate(agent_dist):
                if index != self.robot_index and dist < self.blackboard.local_comms_range:
                    neighbour_task_log = self.get_neighbour_task_data(index)

                    # Compare task logs and update if needed
                    for id in neighbour_task_log: 
                        status = neighbour_task_log[id]['status']
                        carry_status = neighbour_task_log[id]['carry_status']
                        colour = neighbour_task_log[id]['colour']

                        if self.blackboard.local_task_log[id]['status'] != status:
                            if status == int(1):
                                # Update internal task log
                                self.blackboard.local_task_log[id]['status'] = status
                        
                        if self.blackboard.local_task_log[id]['carry_status'] != carry_status:

                            # print(f'{index}, {colour}, {carry_status}')

                            # If we do not have a carry status entry, take neighbours information
                            if self.blackboard.local_task_log[id]['carry_status'] == [0, 0]:

                                # print(f' {self.robot_index}  updateee', {index})

                                self.blackboard.local_task_log[id]['carry_status'] = carry_status
                            
                            # If we do have a carry status entry, compare to neighbours, whichever started carrying first takes priority
                            if carry_status[1] < self.blackboard.local_task_log[id]['carry_status'][1]:
                                # Update internal task log
                                self.blackboard.local_task_log[id]['carry_status'] = carry_status
...
class update_hive_mind(py_trees.behaviour.Behaviour): 
    ...
    def get_task_data(self):
        ...
        if response.status == 200:
            ...
            task_dict = [dict(item) for item in task_data] 
            return task_dict
        else:
    ...
    def get_task_data_fake(self):
        '''Requests data from fake Hive Mind. Includes drop out rate. Sends data to the recv buffer.'''
        if self.handle_dropout():
            return None
        
        # If no drop out - return task log from Hive Mind with latency applied
        else:
            self.add_to_tick_buffer('recv', type='hm_data_recv', data=self.blackboard.global_task_log)
    
    def update_task_fake(self, id, new_task_status = None, new_carry_status = None):
        if self.handle_dropout():
            pass

        # If no latency or drop out - update the Hive Mind
        else:
            if new_task_status is not None:
                self.add_to_tick_buffer('send', type='hm_data_send', data={"id":id, "data_type":'status', "data": new_task_status})
            if new_carry_status is not None:
                self.add_to_tick_buffer('send', type='hm_data_send', data={"id":id, "data_type":'carry_status', "data": new_carry_status})
    
    def add_to_tick_buffer(self, dir, type, data):
        # Add messages to and from Hive Mind to respective buffers
        msg = Msg(type, data)
    ...
    def update(self):
        ...
        if self.blackboard.use_hm == 'fake_hm':
            ...
            # 1. Request data from the Hive Mind
            ...
            if task_data is None or task_data == 0:
                ...
                for id in task_data:
                    hm_status = task_data[id]['status']
                    hm_carry_status = task_data[id]['carry_status']

                    # Completion status updates - either 0 or 1 depending if box has been delivered yet
                    if self.blackboard.local_task_log[id]['status'] != hm_status:

                        if hm_status == int(1):
                            # Update internal task log
                            self.blackboard.local_task_log[id]['status'] = hm_status

                        elif hm_status == int(0):
                            # Send update to global task log (the fake HM) 
                            self.update_task_fake(id, new_task_status=1)

                        else:
                            print(f'Error - HM status is not 1 or 0 it is: {hm_status} of type {type(hm_status)}')

                    # Progression status updates
                    # If we are targetting this box id - robot name of the robot that has first stated to the Hive that they are carrying a box
                    # Note that with latency it is possible for two robots to both be carrying the same coloured box
                    # In which case the robot that was first to upload their name to carry_status on the HM, has priority
                    if self.blackboard.target_place_id == id:
                        
                        # Check if HM aligns with local task log
                        if hm_carry_status != self.blackboard.local_task_log[id]['carry_status']:

                            # Upate local HM if box being carried by a different robot
                            if hm_carry_status != 0:
                                self.blackboard.local_task_log[id]['carry_status'] = hm_carry_status
                            
                            # Update global HM if box not being carried by a different robot to say we are carrying now
                            elif hm_carry_status == 0:
                    ...
                    elif id in self.blackboard.hm_carry_to_zero:
                    ...
                    elif hm_carry_status != self.blackboard.local_task_log [id]['carry_status']:
                            self.blackboard.local_task_log[id]['carry_status'] = hm_carry_status    
                     
                    # All ids have been checked and reverted to zero where needed so reset blackboard variable
                    self.blackboard.hm_carry_to_zero = []   

                    # 2. Send data to the Hive Mind
                    ...
                    if buffer_msg_send != 0:
                        # In this implementation, we only have one message type we are interested in: HM task updates
                        # There is a Hive side check here to make sure that the update is ok:
                    ...
                    #     print('-------------------------------------')
                    #     for id,value in self.blackboard.global_task_log.items():
                    #         print(f'{id}, {value}')
...
class pre_place(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if self.blackboard.target_place_id is None:
            ...
            self.blackboard.local_task_log[closest_id]['carry_status'][0] = self.str_index
            self.blackboard.local_task_log[closest_id]['carry_status'][1] = time.time()
...
def create_root(robot_index):
    ...
    # Local communication
    local_comm = local_communication(name='Local comms', robot_index=robot_index)
    ...
    DOTS_actions.add_child(py_trees.decorators.Inverter(local_comm))

...
class update_global(py_trees.behaviour.Behaviour):
    ...
    def setup(self):
        ...
        self.logger.debug(f"Select update_global::setup {self.name}")

    def initialise(self):
        self.logger.debug(f"select update_global::initialise {self.name}")
    ...
    def add_to_tick_buffer(self, dir, type, data):
        # Add messages to and from global Mind to respective buffers
        msg = Msg(type, data)
    ...
    def update(self):
        ...
        if self.blackboard.use_global == 'global':
            ...
            # 1. Request data from the global Mind
...
def create_root(robot_index):
    ...
    # Select actiontalk_to_global(name='Talk to global', robot_index=robot_index)

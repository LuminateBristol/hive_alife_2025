...
class select_action(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name, robot_index):
        ...
        self.blackboard.register_key(key="global_mode", access=py_trees.common.Access.READ)
        self.setup()
    ...
    def handle_pre_place(self):
        box = self.blackboard.boxes[self.blackboard.target_box]

        # Check task log to see if item still needs placing
    ...
    def handle_abandon(self):
        box = self.blackboard.boxes[self.blackboard.target_box]
        if (self.blackboard.rob_c[self.robot_index][0] < self.blackboard.abandon_tol or
                self.blackboard.rob_c[self.robot_index][0] > self.blackboard.map.width - self.blackboard.abandon_tol or
                self.blackboard.rob_c[self.robot_index][1] < self.blackboard.abandon_tol or
                self.blackboard.rob_c[self.robot_index][1] > self.blackboard.map.height - self.blackboard.abandon_tol):
            self.blackboard.action = 'random_walk'
            self.blackboard.target_box = None
            self.blackboard.target_place_id = None
            box.action_status = 0
...
class local_communication(py_trees.behaviour.Behaviour):
    ...
    def update_task_status(self, id, new_task_status):
...
class update_global(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name, robot_index):
        ...
        self.blackboard.register_key(key='global_session_id', access=py_trees.common.Access.WRITE)
    ...
    def setup(self):
        self.blackboard.global_session_id = 0
    ...
    def update_task_fake(self, id, new_task_status):
        if self.handle_dropout():
            pass
    ...
    def update(self):
        # Using fake global
        if self.blackboard.use_global == 'fake_global':
            ...
            if task_data is None or task_data == 0:
                ...
                for id, value in task_data.items(): 
                    colour = value['colour']
                    ...
                    if self.blackboard.local_task_log[id]['status'] != global_status:
                        ...
                        if global_status == int(1):
                            ...
                            self.update_task_fake(id, 1)
                        else:
                            print(f'Error - global status is not 1 or 0 it is: {global_status} of type {type(status)}')
...
def create_root(robot_index):
    ...
    # Select actiontalk_to_hive_mind(name='Talk to global', robot_index=robot_index)

...
class Robot:
    ...
    def add_observations(self):
        ...
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_led_status',    'has_status',   {'type':'robot_led_status',      'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':[],                          'time':0} ])
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_lifter_status', 'has_status',   {'type':'robot_lifter_status',   'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':False,                       'time':0} ])
        ...
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_speed',         'has_status',   {'type':'robot_speed_status',    'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':0,                           'time':0} ])
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_task_id',       'in_progress',  {'type':'robot_task_id',         'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':0,                           'time':0} ])
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_pheromone_map', 'has_status',   {'type':'robot_pheromone_map',   'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':{},                          'time':0} ])
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_chosen_door',   'has_status',   {'type':'chosen_door',           'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':0,                           'time':0} ])
        self.observation_space.append([f'robot_{self.robot_index}', f'robot_{self.robot_index}_waypoint',      'in_progress',  {'type':'waypoint',              'weight':1, 'robot_name':f'robot_{self.robot_index}',  'data':0,                           'time':0} ])
    ...
    def build_robo_mind(self, entities, tasks, map=None):
        ...
        # Register a copy of the robo mind as the first part of the distributed_hive_mind graph - i.e. so far only populated by us
        self.blackboard.register_key(key="distributed_hive_mind", access=py_trees.common.Access.WRITE)
        self.blackboard.distributed_hive_mind = deepcopy(self.robo_mind)
    ...
    def add_comms_db(self, comms_db):
        """
        Sets up the link to the comms_db - a database of all robot graphs used to simulate the communication of one robots graph to another robot

        Args:
            comms_db (class object): Distributed mind object - see hive_mind.py and initalised in sim.py
        """
        # Setup global graph object
        self.blackboard.register_key(key="comms_db", access=py_trees.common.Access.WRITE)
        self.blackboard.comms_db = comms_db
...
class Swarm_Decentralised:
    """
    Represents a decentralised multi-robot system in which case robots only share information by broadcasting it to one another
    if within a given communication range.

    To achieve this, the Hive_Mind object is
    """
    ...
    def add_agents(self, agent_obj, comms_db=None, processed_delivery_points=None, traffic_score=None):
        """
        Adds a specified number of robot agents to the swarm, initializing their behavior trees and blackboards.
        ...
        for ag in self.agents:
            ...
            # Set up Robo Mind and Distributed Mind
            try:
                entities = self.exp_cfg.get('entities')
            except:
                print('No entities defined in config')
            ...
            ag.build_robo_mind(entities, tasks)
            if comms_db is not None:
                comms_db.add_robot_observation_space(ag.observation_space)
            ag.add_comms_db(comms_db)
